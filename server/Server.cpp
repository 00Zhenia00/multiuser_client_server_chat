...
void Server::clientHandler(int connectionId) {
    ...
    while (true) {
        Message inmsg;
        size_t length = boost::asio::read(sock, boost::asio::buffer(&inmsg, sizeof(inmsg)), error);
        if (error) {
            if (error == boost::asio::error::eof) {
                std::cout << "Server::clientHandler(): Client '"
        ...
        case MessageType::CONNECT: {
            ...
            boost::asio::write(sock, boost::asio::buffer(reinterpret_cast<void*>(&outmsg), sizeof(outmsg)), error);
            if (error) {
                std::cout << "Server::clientHandler(): Failed to write"
                    "message to client '" << connectionId << "'!\n";
                return;
            }
            break;
        }
        ...
        case MessageType::PING: {
            boost::asio::write(sock, boost::asio::buffer(reinterpret_cast<void*>(&inmsg), sizeof(inmsg)), error);
            if (error) {
                std::cout << "Server::clientHandler(): Failed to write"
                    "message to client '" << connectionId << "'!\n";
                return;
...
void Server::run() {
    ...
    while (true) {
        ...
        auto connection = std::make_unique<ClientConnection>(mClientIdCounter, "Unknown user", std::move(sock));
